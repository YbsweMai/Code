<template>
  <div>
    <el-table
      v-loading="loading"
      :data="restList && restList.list && restList.list.slice((currentPage-1)*pageSize,currentPage*pageSize)"
    >
      <el-table-column
        prop="name.zh-CN"
        label="餐馆"
...
export default {
   ...
   methods:{
      ...
      async changeClosed (v, index, row){
         ...
         //     // 你上面渲染的数据是 处理之后的数组，
         //     // 那这个下标对应是处理之后的数组，，那只能再获取一下，处理之后的数组，才能修改到对应到下标到那一项，
