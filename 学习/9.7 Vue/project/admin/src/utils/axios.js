...
const get = async (url,data) => {
    let response = await api.get(url,data).catch( error => {  /* .then可以省略不写 */
        errorHandler(error)
    })
...
const post = async (url,data) => {
    let response = await api.post(url,data).catch( error => {  /* .then可以省略不写 */
        errorHandler(error)
    })
    return responseHandler(response)
}
...
const put = async (url,data) => {
    let response = await api.put(url,data).catch( error => {  /* .then可以省略不写 */
        errorHandler(error)
    })
    return responseHandler(response)
}
...
const responseHandler = response => {
    ...
    }else {   /* 此处else可以不写 因为上面判断后返回就不会再执行 */
        return null
    }
}
