...
1. 给定insert语句
   ...
   ```
   db.banyuans.updateMany({role:'student'},{$set:{study:{
   ...
   ```
   db.banyuans.find({role:'student'}).count()
   ```

   找到年龄小于24的学生人数

   ```
   db.banyuans.find({role:'student',age:{$lt:24}}).count()
   ```

   找到年龄小于等于20的学生，增加一个字段为important，值为true，意义在于年龄小需要重点关注，同时更新大于20的学生，important为false

   ```
   db.banyuans.updateMany({role:'student',age:{$lte:20}},{$set:{important:true}});
   ```
